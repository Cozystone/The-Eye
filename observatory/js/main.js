...
import { MapperOverlay } from './mapper-overlay.js';
...
class Observatory {
  ...
  _adaptLiveData(data, elapsed) {
    if (!data || !String(data.source || '').startsWith('wifi:')) {
      return data;
    }
    ...
    const presence = this._inferProxyPresence(adapted);
    adapted.classification.presence = presence;
    adapted.classification.confidence = this._inferProxyConfidence(adapted, presence);
    adapted.estimated_persons = presence ? Math.max(1, adapted.estimated_persons || 0) : 0;
  ...
  _inferProxyPresence(data) {
    if (data?.classification?.presence) return true;

    const quality = Number(data.signal_quality_score || 0);
    const bssidCount = Number(data.bssid_count || 0);
    ...
    const breathing = Number(data?.vital_signs?.breathing_rate_bpm || 0);
  ...
  _inferProxyConfidence(data, isPresent) {
    const base = Number(data?.classification?.confidence || 0);
    const quality = Number(data.signal_quality_score || 0);
    const bssidFactor = Math.min(1, Number(data.bssid_count || 0) / 12);
    const variance = Math.min(1, Number(data?.features?.variance || 0) * 8);
    const motion = Math.min(1, Number(data?.features?.motion_band_power || 0) * 25);
    const breathing = Number(data?.vital_signs?.breathing_rate_bpm || 0) >= 6 ? 0.12 : 0;
    const derived = 0.2 + quality * 0.35 + bssidFactor * 0.18 + variance * 0.15 + motion * 0.12 + breathing;
    const confidence = Math.max(base, derived);
    return isPresent ? Math.min(0.9, confidence) : Math.min(0.55, confidence * 0.6);
  }

  _buildProxyPerson(data, elapsed) {
    const pos = this._estimateProxyPosition(data, elapsed);
    const motion = Number(data?.features?.motion_band_power || 0);
    const variance = Number(data?.features?.variance || 0);
    const motionScore = Math.max(8, Math.round((motion * 1400) + (variance * 180)));
    const pose = motion > 0.03 ? 'walking' : 'standing';
    this._proxyPersonState.confidence = Number(data?.classification?.confidence || 0.35);

    return {
      id: 'wifi-proxy-person',
      position: [pos.x, 0, pos.z],
      pose,
      facing: this._proxyPersonState.heading,
      motion_score: motionScore,
      confidence: this._proxyPersonState.confidence,
    };
  }

  _estimateProxyPosition(data, elapsed) {
    const rawFieldValues = data?.signal_field?.values;
    const fieldValues = Array.isArray(rawFieldValues) || ArrayBuffer.isView(rawFieldValues)
      ? rawFieldValues
      : [];
    const gridSize = Array.isArray(data?.signal_field?.grid_size) ? data.signal_field.grid_size : [20, 1, 20];
    let sum = 0;
    let weightedX = 0;
    let weightedZ = 0;
    const width = Math.max(1, Number(gridSize[0] || 20));
    const depth = Math.max(1, Number(gridSize[2] || 20));

    for (let i = 0; i < fieldValues.length; i++) {
      const value = Number(fieldValues[i] || 0);
    ...
    let targetZ = 0;
    if (sum > 0) {
      targetX = ((weightedX / sum) / Math.max(1, width - 1) - 0.5) * 8;
      targetZ = ((weightedZ / sum) / Math.max(1, depth - 1) - 0.5) * 6;
    } else {
      const sway = Math.sin(elapsed * 0.35) * 0.18;
      targetX = sway;
      targetZ = Math.cos(elapsed * 0.22) * 0.12;
    }

    const variance = Number(data?.features?.variance || 0);
    const motion = Number(data?.features?.motion_band_power || 0);
    targetX += Math.sin(elapsed * 0.9) * Math.min(0.2, variance * 3.5);
    targetZ += Math.cos(elapsed * 0.7) * Math.min(0.25, motion * 6);

    this._proxyPersonState.x += (targetX - this._proxyPersonState.x) * 0.08;
    this._proxyPersonState.z += (targetZ - this._proxyPersonState.z) * 0.08;
    this._proxyPersonState.heading = Math.atan2(targetX - this._proxyPersonState.x, targetZ - this._proxyPersonState.z || 0.001);
    return this._proxyPersonState;
  }

  _buildProxySignalField(data) {
    const existing = data?.signal_field;
    const rawExistingValues = existing?.values;
    const existingValues = Array.isArray(rawExistingValues) || ArrayBuffer.isView(rawExistingValues)
      ? rawExistingValues
      : [];
    const hasExistingField = Array.from(existingValues).some((value) => Number(value || 0) > 0.01);
    if (hasExistingField) {
      return existing;
    }
    ...
    const width = 20;
    const depth = 20;
    const centerX = ((this._proxyPersonState.x / 8) + 0.5) * (width - 1);
    const centerZ = ((this._proxyPersonState.z / 6) + 0.5) * (depth - 1);
    const sigma = 2.2;
    const values = [];
    ...
    for (let z = 0; z < depth; z++) {
      for (let x = 0; x < width; x++) {
        ...
        values.push(Math.exp(-distance2 / (2 * sigma * sigma)) * 0.85);
      }
    }

    return {
      grid_size: [width, 1, depth],
      values,
    };
  }
